/**
 * Vendored from babel-plugin-polyfill-corejs3 (built-in-definitions.js).
 * This is spec-shape metadata -- which AST pattern (global identifier /
 * static property / instance method) maps to which core-js-compat module
 * id(s) -- not core-js implementation code, same "data not code" distinction
 * already used for core-js-compat itself. Reused as-is so spackle does not
 * hand-author its own copy of this ~500-entry table for usage-global support.
 *
 * The `pure` field (first arg to define()) is core-js-pure feature paths --
 * unused by spackle today (no usage-pure support yet), kept for a future
 * usage-pure implementation.
 */
// @ts-nocheck -- vendored data, mechanically converted from JS; not worth
// hand-annotating a ~500-entry table just to satisfy strict mode.

// Plain `import` (esbuild inlines JSON at build time via tsup's noExternal)
// -- see provider.ts for the full explanation.
import compatDataImport from 'core-js-compat/data.json';
const compatData: Record<string, any> = compatDataImport;
const _extends = Object.assign;
const polyfillsOrder: Record<string, number> = {};
Object.keys(compatData).forEach((name, index) => {
...
const StaticProperties = {
  ...
  RegExp: {
    // spackle ports RegExp.escape under the stable es.regexp.escape id (present
    // in core-js-compat data); the esnext id would never inject since spackle
    // has no esnext ids and no esnext fallback.
...
const InstanceProperties = {
  ...
  // ES2024 Set methods -- absent from the upstream vendored table; added so
  // usage-global resolves a bare `set.union(...)` etc. to spackle's ported
  // es.set.*.v2 modules. Names are Set-unique, so no receiver disambiguation
  // is needed.
  union: define(null, ["es.set.union.v2"]),
  intersection: define(null, ["es.set.intersection.v2"]),
  difference: define(null, ["es.set.difference.v2"]),
  symmetricDifference: define(null, ["es.set.symmetric-difference.v2"]),
  isSubsetOf: define(null, ["es.set.is-subset-of.v2"]),
  isSupersetOf: define(null, ["es.set.is-superset-of.v2"]),
  isDisjointFrom: define(null, ["es.set.is-disjoint-from.v2"]),
