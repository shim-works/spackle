...
export const isSupported = (): boolean => {
  try {
    ...
    return !!table && table.flat === true;
  } catch {
    return true;
  }
};

export const isArrayUnscopablesFlatSupported = isSupported;

/**
 * Spec: https://tc39.es/ecma262/#sec-array.prototype-@@unscopables
 *
 * Keeps `flat` out of `with (array) { ... }` scope, so old code with a local
 * `flat` binding isn't shadowed by the array method.
 *
